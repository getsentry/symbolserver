...
def find_executable():
    if TARGET:
        path = os.path.join('target', TARGET, BIN_TYPE, 'sentry-symbolserver' + EXT)
        if os.path.isfile(path):
            return path
    path = os.path.join('target', BIN_TYPE, 'sentry-symbolserver' + EXT)
    if os.path.isfile(path):
        return path
...
def get_target_executable_name():
    ...
    arch = bits[0]
    return 'sentry-symbolserver-%s-%s%s' % (platform, arch, EXT)
...
def ensure_release():
    ...
    resp = api_request('POST', 'repos/%s/releases' % REPO, json={
        'tag_name': TAG,
        'name': 'sentry-symbolserver %s' % TAG,
        'draft': True,
    })
    resp.raise_for_status()
    release = resp.json()
